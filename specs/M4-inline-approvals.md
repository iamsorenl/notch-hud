...
- **Never gate background sessions**: if the hook process has no tty ancestor (reuse the `find_tty` ppid-walk from notch-emit) → instant no-op. Cooper's launchd agents (email-triage, CooperBrain) must never stall.
- Feature flag: `~/.notch-hud/config.json` `{"approvals": true}` — if file missing or false → instant no-op. Installer writes it true.
- Only gate when `permission_mode == "default"` and tool ∈ {Edit, Write, NotebookEdit, Bash}. Everything else → instant no-op.

## 1. New hook script `scripts/notch-approve` (POSIX sh + jq, synchronous — NOT async)
...
2. Run the no-op checks (flag, tty, mode, tool set). Also: if `~/.notch-hud/session-allow/<session_id>` exists and contains a line matching the tool class (see Bypass below) → output `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","permissionDecisionReason":"NotchHUD bypass"}}` and exit.
...
4. Write pending file `~/.notch-hud/pending/<session_id>.json` (atomic tmp+mv):
```json
...
5. Poll `~/.notch-hud/decisions/<session_id>.json` every 0.25s up to 20s. On decision `{"decision":"allow"|"deny","scope":"once"|"session"}`:
   - allow → output permissionDecision allow (reason "Approved from NotchHUD"); if scope=session append the tool class line to `~/.notch-hud/session-allow/<session_id>` (`Bash:<first word of command>` or the tool name).
   - deny → output permissionDecision deny (reason "Denied from NotchHUD").
   - Always delete the pending + decision files.
6. Timeout → delete pending file, exit 0 silently.

## 2. App: pending watcher + approval card
- **`PendingStore.swift` + reuse of the watcher:** generalize `SpoolWatcher` (init takes directory + onRescan callback) or add a sibling `PendingWatcher` on `~/.notch-hud/pending/` (create dir in AppEnvironment). Decodes `PendingApproval` model.
...
- **`ApprovalCardView.swift`** (match the reference):
  ...
  - Button action → write `~/.notch-hud/decisions/<sessionId>.json` (atomic) with the decision + scope, then optimistically dismiss the card.
...
`notch-claude-hook` gains `notify` mode: reads Notification payload (`.message`, `.title` or similar), emits status `needs_me` with `--detail "<message first 80 chars>"`. (Manager registers it on the `Notification` event.) A subsequent PreToolUse/UserPromptSubmit/Stop naturally flips the status back.
