...
EXPANDS=$(grep -c "expand() called" ~/.notch-hud/hover.log 2>/dev/null || echo 0)
echo "expand triggers: $ENTERS"
EXITS=$(grep -c "EXIT delivered" ~/.notch-hud/hover.log 2>/dev/null || echo 0)
