...
struct ApprovalCardView: View {
    ...
    private func decide(_ decision: ApprovalDecision.Decision, scope: ApprovalDecision.Scope) {
        do {
            ...
            NSLog("NotchHUD could not write approval decision: %@", error.localizedDescription)
        }
    }
    ...
    private func bypass() {
        do {
            ...
            do {
                ...
                NSLog(
                    "NotchHUD could not prewrite the session allowance: %@",
                    error.localizedDescription
                )
            }
            onDismiss(approval.sessionId)
        } catch {
            NSLog("NotchHUD could not write bypass decision: %@", error.localizedDescription)
        }
    }
