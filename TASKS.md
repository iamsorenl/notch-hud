...
- [x] `SessionIndex` — head-only transcript scan (~/.claude/projects), pure parse/plan + fixtures
- [x] `ResumeLauncher` — Terminal.app new tab `claude --resume`, clipboard fallback on denied automation
- [x] `RecentsSectionView` + `RecentSessionRowView` — filter chips (All · per-project · +N), cap 8, muted styling
- [x] Wire into `NotchPanelView`; lazy refresh on expand
